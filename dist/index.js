...
const wrapNodes = (editor, element, options = {}) => {
  unhangRange(editor, options);
  return slate.Transforms.wrapNodes(editor, element, options);
};
...
const ToolbarBase = /*#__PURE__*/React.forwardRef((_a, ref) => {
  var {
    className,
    styles,
    children
...
const getListItemEntry = (editor, {
  ...
  if (at && someNode(editor, {
    ...
    if (slate.Range.isRange(at) && !isCollapsed(at) && paragraphPath.length === 1) {
      at = at.focus.path;
    }
...
const moveListItemUp = (editor, {
  ...
  const move = () => {
    ...
    if (!liParent) {
      const toListPath = slate.Path.next(listPath);
  ...
  if (editor.selection) {
    ...
    slate.Editor.withoutNormalizing(editor, () => {
      ...
      slate.Transforms.wrapNodes(editor, {
        ...
        at: listItemPath,
        match: (n, p) => p.length === nextParagraphPath.length && !slate.Path.isBefore(p, nextParagraphPath)
      });
      slate.Transforms.moveNodes(editor, {
        at: nextParagraphPath,
        to: nextListItemPath
...
const autoformatBlock = (editor, type, at, {
  ...
  const selectionRef = slate.Editor.rangeRef(editor, editor.selection);
  slate.Transforms.delete(editor, {
    at
  });
  ...
  if (!selectionRef.current) {
    ...
    const nextPosition = slate.Editor.after(editor, editor.selection.anchor);
    slate.Transforms.select(editor, nextPosition);
  } else {
    selectionRef.unref();
  }
