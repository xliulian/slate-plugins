...
var imageExtensions__default = /*#__PURE__*/_interopDefaultLegacy(imageExtensions);
...
const getQueryOptions = (editor, options) => {
  return Object.assign(Object.assign({}, options), {
    match: n => match(n, options.match) && (!(options === null || options === void 0 ? void 0 : options.block) || slate.Editor.isBlock(editor, n))
  });
};
...
const findNode = (editor, options) => {
  ...
  try {
    ...
    const nodeEntries = slate.Node.nodes(root[0], {
      reverse,
      from,
      to,
      pass: ([n]) => voids ? false : slate.Editor.isVoid(editor, n)
...
const isRangeAcrossBlocks = (editor, _a = {}) => {
  ...
  const endBlock = slate.Editor.above(editor, Object.assign({
    at: end
  }, options));
...
const getOnHotkeyToggleMark = ({
  ...
  if (!hotkey) return;
  return (e, editor) => {
    if (hotkey && isHotkey__default['default'](hotkey, e)) {
      e.preventDefault();
...
const isWordAfterTrigger = (editor, {
  ...
  const beforeRegex = new RegExp(`^${escapedTrigger}([\\w|À-ÖØ-öø-ÿ]+)$`); // Match regex on before text
...
const mergeNodes = (editor, options = {}) => {
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    const emptyAncestor = slate.Editor.above(editor, {
      at: path,
      mode: 'highest',
      match: n => levels.includes(n) && slate.Element.isElement(n) && n.children.length === 1
    });
    ...
    if (slate.Element.isElement(prevNode) && slate.Editor.isEmpty(editor, prevNode) || slate.Text.isText(prevNode) && prevNode.text === '') {
      slate.Transforms.removeNodes(editor, {
        at: prevPath,
        voids
      });
...
const ToolbarButtonBase = ({
  ...
  return tooltip ? /*#__PURE__*/React.createElement(Tippy__default['default'], Object.assign({}, tooltipProps), button) : button;
};
...
const decoratePreview = () => ([node, path]) => {
  ...
  const tokens = Prism__default['default'].tokenize(node.text, Prism__default['default'].languages.markdown);
  let start = 0;
...
const SelectableBase = ({
  ...
  }, children, !!dropLine && /*#__PURE__*/React__default['default'].createElement("div", {
  ...
  }, /*#__PURE__*/React__default['default'].createElement(Tippy__default['default'], Object.assign({}, grabberTooltipProps), /*#__PURE__*/React__default['default'].createElement("div", {
    ref: multiDragRef
...
const getSelectableElement = ({
  ...
  return /*#__PURE__*/React.forwardRef((_a, ref) => {
    ...
    return /*#__PURE__*/React__default['default'].createElement(Selectable, {
      attributes: attributes,
      componentRef: ref,
      element: element,
      styles: styles,
  ...
  return editor => {
    ...
    editor.insertData = data => {
      ...
      if (html) {
        ...
        if (fragment.some(f => slate.Element.isElement(f) || Object.keys(f).some(k => k !== 'text'))) {
...
const CodeLineElement = utilities.styled(CodeLineElementBase, getCodeLineElementStyles, undefined, {
  scope: 'CodeBlockElement'
});
...
const toggleCodeBlock = (editor, options) => {
  ...
  if (!isActive) {
    ...
    for (const [, path] of nodes) {
      ...
      slate.Transforms.setNodes(editor, codeLine, {
        at: path
      });
    }
  }
...
const isImageUrl = url => {
  ...
  return imageExtensions__default['default'].includes(ext);
};
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    deleteFragment(editor, {
      ...
      moveNode: (_editor, {
        ...
        if (!editor.selection) return;
        const [, path] = slate.Editor.node(editor, at);
        const blockAbove = getBlockAbove(editor, {
...
const removeFirstListItem = (editor, {
  ...
  if (!isListNested(editor, listPath, options) && listNode.children.length <= 1 && hasListChild(listItemNode) && isFirstChild(listItemPath)) {
...
const matchesTriggerAndPattern = (editor, {
  ...
  const lineStart = slate.Editor.before(editor, at, {
    unit: 'line'
  }); // Range from before to start

  const beforeRange = lineStart && slate.Editor.range(editor, lineStart, at); // Before text
  ...
  const mentionStart = match ? slate.Editor.before(editor, at, {
    unit: 'character',
    distance: match[1].length + trigger.length
  }) : null; // Range from mention to start

  const mentionRange = mentionStart && slate.Editor.range(editor, mentionStart, at);
  return {
    range: mentionRange,
    match
  };
...
const getClassNames$g = utilities.classNamesFunction();
/**
...
const onKeyDownExitBreak = ({
  ...
  rules.forEach(({
    ...
    if (isHotkey__default['default'](hotkey, event) && queryNode(entry, query)) {
      if (!editor.selection) return;
      const {
        queryEdge,
        isEdge,
...
exports.ATTRIBUTE_LINK = ATTRIBUTE_LINK;
...
exports.CodeLineElementBase = CodeLineElementBase;
...
exports.ELEMENT_ALIGN_JUSTIFY = ELEMENT_ALIGN_JUSTIFY;
...
exports.matchesTriggerAndPattern = matchesTriggerAndPattern;
