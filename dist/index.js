...
var useMergedRef = _interopDefault(require('@react-hook/merged-ref'));
var Tippy = _interopDefault(require('@tippyjs/react'));
var styling = require('@uifabric/styling');
var utilities = require('@uifabric/utilities');
var reactDndHtml5Backend = require('react-dnd-html5-backend');
var reactDnd = require('react-dnd');
var isHotkey = _interopDefault(require('is-hotkey'));
var ReactDOM = require('react-dom');
var Prism$1 = require('prismjs');
var Prism$1__default = _interopDefault(Prism$1);
var server = require('react-dom/server');
var slateHyperscript = require('slate-hyperscript');
var markdown = _interopDefault(require('remark-parse'));
var slate$1 = _interopDefault(require('remark-slate'));
...
const getAboveByType = (editor, types, options = {}) => {
  types = castArray_1(types);
  return slate.Editor.above(editor, Object.assign({
...
const getBlockAbove = (editor, options = {}) => slate.Editor.above(editor, Object.assign({
...
const getLastChildPath = nodeEntry => getLastChild(nodeEntry)[1];

const isAncestor = node => slate.Element.isElement(node) || slate.Editor.isEditor(node);
...
const getNodesById = (editor, id, editorNodesOptions) => {
  const ids = castArray_1(id);
  const nodes = slate.Editor.nodes(editor, Object.assign({
    mode: 'all',
    match: n => {
      const nodeId = n.id;
...
const getNodeById = (editor, id, editorNodesOptions) => {
  var _a;

  return (_a = getNodesById(editor, id, editorNodesOptions)) === null || _a === void 0 ? void 0 : _a[0];
};
...
const getBlockPathById = (editor, id) => {
...
const useDndBlock = ({
  ...
  const editor = slateReact.useEditor();
...
const GrabberTooltipContent = () => /*#__PURE__*/React__default.createElement("div", {
  style: {
    fontSize: 12
  }
}, /*#__PURE__*/React__default.createElement("div", null, "Drag ", /*#__PURE__*/React__default.createElement("span", {
  style: {
...
const grabberTooltipProps = {
  content: /*#__PURE__*/React__default.createElement(GrabberTooltipContent, null),
  placement: 'bottom',
  arrow: false,
  offset: [0, 0],
  delay: [300, 0],
...
const getClassNames = utilities.classNamesFunction();
...
const SelectableBase = ({
  ...
  return /*#__PURE__*/React__default.createElement("div", {
    ...
    ref: blockRef,
    className: styling.mergeStyles(classNames.blockAndGutter, classNames.block)
  }, children, !!dropLine && /*#__PURE__*/React__default.createElement("div", {
    className: classNames.dropLine,
    contentEditable: false
  })), /*#__PURE__*/React__default.createElement("div", {
    className: styling.mergeStyles(classNames.blockAndGutter, classNames.gutterLeft),
    contentEditable: false
  }, /*#__PURE__*/React__default.createElement("div", {
    className: classNames.blockToolbarWrapper
  }, /*#__PURE__*/React__default.createElement("div", {
    className: classNames.blockToolbar
  }, /*#__PURE__*/React__default.createElement(Tippy, Object.assign({}, grabberTooltipProps), /*#__PURE__*/React__default.createElement("div", {
    ref: multiDragRef
...
const Selectable = utilities.styled(SelectableBase, getSelectableStyles, undefined, {
  scope: 'Selectable'
});
...
const getSelectableElement = ({
  ...
  return /*#__PURE__*/React.forwardRef((_a, ref) => {
    var {
      attributes,
      element
    } = _a,
    ...
    return /*#__PURE__*/React__default.createElement(Selectable, {
      attributes: attributes,
      componentRef: ref,
      element: element,
      styles: styles,
...
const getBlocks = (editor, options) => {
  return [...slate.Editor.nodes(editor, Object.assign({
...
const selectBlocksBySelectionOrId = (editor, id) => {
  ...
  const blockEntries = getBlocks(editor, {
    at: editor.selection
  });
...
const isNodeType = (entry, {
  ...
  if (allow.length) {
    filterAllow = ([n]) => allow.includes(n.type);
  }

  let filterExclude = () => true;

  if (exclude.length) {
    filterExclude = ([n]) => !exclude.includes(n.type);
  }
...
const getPreviousBlockById = (editor, id, query) => {
  ...
  const firstNodeEntry = [...slate.Editor.nodes(editor, {
    mode: 'highest',
    match: n => {
      return slate.Editor.isBlock(editor, n) && n.id && isNodeType([n, []], query);
    },
    at: []
  })];
...
const getText = (editor, at) => {
  ...
  return (_a = at && slate.Editor.string(editor, at)) !== null && _a !== void 0 ? _a : '';
};
...
const isDescendant = node => slate.Element.isElement(node) || slate.Text.isText(node);
...
const isSelectionAtBlockEnd = editor => {
  ...
  return isEnd(editor, (_a = editor.selection) === null || _a === void 0 ? void 0 : _a.focus, path);
};
...
const isSelectionAtBlockStart = editor => {
  ...
  const [, path] = getBlockAbove(editor);
  return isStart(editor, (_a = editor.selection) === null || _a === void 0 ? void 0 : _a.focus, path);
};
...
const normalizeDescendantsToDocumentFragment = descendants => {
  descendants.forEach(element => {
    if (slate.Element.isElement(element)) {
  ...
  editor.apply = operation => {
    if (operation.type === 'insert_node') {
      const newFilter = entry => {
        const [node] = entry;
        return filter(entry) && filterText ? slate.Element.isElement(node) : isDescendant(node);
      };
...
const getClassNames$1 = utilities.classNamesFunction();
/**
...
const StyledElement = utilities.styled(StyledElementBase, getStyledComponentStyles, undefined, {
  scope: 'StyledElement'
});
...
const unwrapNodesByType = (editor, types, options = {}) => {
  types = castArray_1(types);
  slate.Transforms.unwrapNodes(editor, Object.assign({
    match: n => types.includes(n.type)
  }, options));
};
...
const getClassNames$2 = utilities.classNamesFunction();
/**
...
const StyledComponent = utilities.styled(StyledComponentBase, getStyledComponentStyles, undefined, {
  scope: 'StyledComponent'
});

const getClassNames$3 = utilities.classNamesFunction();
/**
...
const StyledLeaf = utilities.styled(StyledLeafBase, getStyledComponentStyles, undefined, {
  scope: 'StyledLeaf'
});
...
const getToolbarStyles = utilities.memoizeFunction(styles => {
  return styling.concatStyleSets({
...
const getClassNames$4 = utilities.classNamesFunction();
const ToolbarBase = /*#__PURE__*/React.forwardRef(({
  className,
  styles,
  children
}, ref) => {
...
const Toolbar = /*#__PURE__*/React.forwardRef((_a, ref) => {
  var {
    styles
  } = _a,
      props = __rest(_a, ["styles"]);

  return /*#__PURE__*/React.createElement(ToolbarBase, Object.assign({}, props, {
    ref: ref,
    styles: styling.concatStyleSets(getToolbarStyles(styles))
  }));
});
...
const getBalloonToolbarStyles = utilities.memoizeFunction((className, styles, theme, hidden, hiddenDelay, direction, arrow) => {
...
const BalloonToolbar = ({
  ...
  return /*#__PURE__*/React.createElement(PortalBody, null, /*#__PURE__*/React.createElement(Toolbar, {
    ref: ref,
...
const getHeadingToolbarStyles = ({
  ...
  return styling.concatStyleSets(getToolbarStyles(), styles);
};

const HeadingToolbar = utilities.styled(ToolbarBase, getHeadingToolbarStyles(), undefined, {
...
const getToolbarButtonStyles = ({
  ...
  }, active && classNames$4.active, className]
});

const getClassNames$5 = utilities.classNamesFunction();
const ToolbarButtonBase = ({
  ...
  const classNames = getClassNames$5(styles, {
    className,
    active
  });
  ...
  const button = /*#__PURE__*/React.createElement("span", Object.assign({
    "data-testid": "ToolbarButton",
    className: classNames.root
  }, spanProps), icon);
  return tooltip ? /*#__PURE__*/React.createElement(Tippy, Object.assign({}, tooltipProps), button) : button;
};
const ToolbarButton = utilities.styled(ToolbarButtonBase, getToolbarButtonStyles, undefined, {
  scope: 'ToolbarButton'
});
...
const ToolbarElement = _a => {
  ...
  const editor = slateReact.useSlate();
  return /*#__PURE__*/React.createElement(ToolbarButton, Object.assign({
...
const getClassNames$6 = utilities.classNamesFunction();
/**
...
const PreviewLeafBase = ({
  ...
  const classNames = getClassNames$6(styles, Object.assign({
    className
  }, leaf));
...
const PreviewLeaf = utilities.styled(PreviewLeafBase, getPreviewLeafStyles, undefined, {
  scope: 'PreviewLeaf'
});
...
const decoratePreview = () => ([node, path]) => {
  ...
  const tokens = Prism$1__default.tokenize(node.text, Prism$1__default.languages.markdown);
  let start = 0;
...
const renderLeafPreview = () => props => /*#__PURE__*/React.createElement(PreviewLeaf, Object.assign({}, props));
...
const createElementWithSlate = slateProps => {
  ...
  return /*#__PURE__*/React.createElement(slateReact.Slate, Object.assign({
    editor,
    value,
    onChange
  }, props), children);
...
const getNode$1 = ({
  ...
  plugins.some(plugin => {
    ...
    html = server.renderToStaticMarkup(createElementWithSlate(Object.assign(Object.assign({}, slateProps), {
...
const getLeaf = ({
  ...
  return plugins.reduce((result, plugin) => {
    ...
    let html = decodeURIComponent(server.renderToStaticMarkup(createElementWithSlate(Object.assign(Object.assign({}, slateProps), {
...
const withDeserializeHTML = (_a = {}) => {
  ...
  return editor => {
    ...
    const {
      preInsert = fragment => {
        const inlineTypes = slatePluginsCore.getInlineTypes(plugins);
...
const getClassNames$7 = utilities.classNamesFunction();
/**
...
const BlockquoteElement = utilities.styled(BlockquoteElementBase, getBlockquoteElementStyles, undefined, {
...
const getClassNames$8 = utilities.classNamesFunction();
/**
...
const CodeBlockElementBase = ({
  ...
  return /*#__PURE__*/React.createElement("pre", Object.assign({}, attributes, {
    className: classNames.root
  }, htmlAttributes), /*#__PURE__*/React.createElement("code", null, children));
};
/**
 * CodeBlockElement
 */

const CodeBlockElement = utilities.styled(CodeBlockElementBase, getCodeBlockElementStyles, undefined, {
...
const getClassNames$9 = utilities.classNamesFunction();
/**
...
const LinkElement = utilities.styled(LinkElementBase, getLinkElementStyles, undefined, {
  scope: 'LinkElement'
});
...
const parseMD = options => content => {
  ...
  const tree = unified().use(markdown).use(slate$1, {
    nodeTypes: {
      paragraph: p.type,
...
const getClassNames$a = utilities.classNamesFunction();
/**
...
const TodoListElementBase = ({
  ...
  return /*#__PURE__*/React.createElement("div", Object.assign({}, attributes, {
    className: classNames.root
  }, htmlAttributes), /*#__PURE__*/React.createElement("div", {
    contentEditable: false,
...
const TodoListElement = utilities.styled(TodoListElementBase, getTodoListElementStyles, undefined, {
...
const decorateCodeBlock = () => entry => {
  ...
  if (node.type === ELEMENT_CODE_BLOCK) {
    ...
    const lang = Prism$1.languages[langName]; // if (lang) {

    const tokens = Prism$1.tokenize(text, lang);
    let offset = 0;
...
const deserializeCodeBlock = options => {
  ...
  return {
    ...
    }, (_a = options === null || options === void 0 ? void 0 : options.code_block) === null || _a === void 0 ? void 0 : _a.deserialize))
  };
};
...
const getClassNames$b = utilities.classNamesFunction();
/**
...
const ImageElement = utilities.styled(ImageElementBase, getImageElementStyles, undefined, {
  scope: 'ImageElement'
});
...
const upsertLinkAtSelection = (editor, url, options) => {
  ...
  unwrapNodesByType(editor, link.type, {
    at: editor.selection
  });
  wrapLink(editor, url, {
    link,
...
const withRemoveEmptyNodes = options => editor => {
  ...
  editor.normalizeNode = ([node, path]) => {
    if (node.type && types.includes(node.type) && slate.Node.string(node) === '') {
      slate.Transforms.removeNodes(editor, {
        at: path
      });
    }
...
const unwrapList = (editor, options) => {
  ...
  unwrapNodesByType(editor, [ul.type, ol.type], {
    split: true
  });
};
...
const ToolbarList = _a => {
  ...
  const editor = slateReact.useSlate();
  return /*#__PURE__*/React.createElement(ToolbarElement, Object.assign({
    type: typeList,
...
const normalizeListItem = (editor, {
  ...
  const inlinePathRefs = children.filter(([child]) => !allValidLiChildrenTypes.includes(child.type)).map(([, childPath]) => slate.Editor.pathRef(editor, childPath)); // Ensure that all lists have a <p> tag as a first element
...
const getListItemEntry = (editor, {
  ...
  if (at && isNodeTypeIn(editor, li.type, {
    at
  })) {
    const selectionParent = getParent(editor, at);
    if (!selectionParent) return;
    const [, paragraphPath] = selectionParent;
    const listItem = getAboveByType(editor, li.type, {
      at
...
const isNodeTypeList = (n, options) => {
  const {
    ul,
    ol
  } = setDefaults(options, DEFAULTS_LIST);
  return [ol.type, ul.type].includes(n.type);
};
...
const moveListItemDown = (editor, {
  ...
  const previousSiblingItem = slate.Editor.node(editor, slate.Path.previous(listItemPath));

  if (previousSiblingItem) {
    const [previousNode, previousPath] = previousSiblingItem;
    const sublist = previousNode.children.find(n => isNodeTypeList(n, options));
...
const moveListItemUp = (editor, {
  ...
  const newListItemPath = slate.Path.next(listParentPath); // Move item one level up
...
const hasListInListItem = (listItemNode, options) => listItemNode.children.length > 1 && isNodeTypeList(listItemNode.children[1], options);
...
const getListRoot = (editor, at = editor.selection, options) => {
  ...
  const parentList = getAboveByType(editor, [ul.type, ol.type], {
    at
  });
...
const isListNested = (editor, listPath, options) => {
  ...
  const [listParentNode] = slate.Editor.parent(editor, listPath);
...
const moveListItemSublistItemsToList = (editor, {
  fromListItem,
  toList,
  start
}, options) => {
  ...
  const moved = moveChildren(editor, {
    ...
    to: start ? toList[1].concat([0]) : slate.Path.next(lastChildPath)
...
const moveListSiblingsAfterCursor = (editor, {
  ...
  if (!isNodeTypeList(listNode, options) || slate.Path.isParent(at, to) // avoid moving nodes within its own list
  ) {
      return 0;
    }
...
const deleteListFragment = (editor, selection, options = {}) => {
  const [startSelection, endSelection] = slate.Range.edges(selection); // Selection should contain multiple blocks.

  if (slate.Path.equals(startSelection.path, endSelection.path)) return;
  const root = getListRoot(editor, endSelection, options); // End selection should be in a list.
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    const listEnd = getListItemEntry(editor, {
      at: endSelection
    }, options); // End selection should be in a list item.
    ...
    if (slate.Path.isBefore(startSelection.path, rootPath)) {
      // If start selection is before the root list.
      next = slate.Path.next(rootPath); // Copy the root list after it.
      ...
      childrenMoved = moveListItemSublistItemsToList(editor, {
        ...
        toList: [toListNode, next]
      }, options); // next is the first list item of the root copy.

      next = [...next, 0];
    } else {
      // If start selection is inside the root list.
      // Find the first list item that will not be deleted.
      const listStart = getListItemEntry(editor, {
        at: startSelection
      }, options);
      if (!listStart) return;
      const {
      ...
      next = listItemSublist ? slate.Path.next(getLastChildPath(listItemSublist)) : listItemStart[1].concat([1, 0]);
    ...
    while (slate.Path.isAfter(cursorPath, startSelection.path)) {
      const node = slate.Node.get(editor, cursorPath);

      if (node.type === li.type) {
        siblingsMoved += moveListSiblingsAfterCursor(editor, {
          at: cursorPath,
          to: next
        }, options);
      }

      cursorPath = slate.Path.parent(cursorPath);
...
const removeFirstListItem = (editor, {
  ...
  if (!isListNested(editor, listPath, options) && listNode.children.length <= 1 && hasListInListItem(listItemNode) && isFirstChild(listItemPath)) {
    // move all children to the container
    moveChildren(editor, {
      at: [listItemNode, listItemPath],
      to: slate.Path.next(listPath)
    });
    slate.Transforms.removeNodes(editor, {
      at: listPath
    });
...
const removeRootListItem = (editor, {
  list,
  listItem
}, options) => {
  const [listItemNode, listItemPath] = listItem;
  ...
  const listItemPathRef = slate.Editor.pathRef(editor, listItemPath);
  const previousListItemPath = getPreviousPath(listItemPath);

  if (previousListItemPath) {
    const [previousListItemNode] = slate.Editor.node(editor, previousListItemPath); // We may have a trailing sub-list
    // that we need to merge backwards

    moveListItemSublistItemsToListItemSublist(editor, {
      fromListItem: listItem,
      toListItem: [previousListItemNode, previousListItemPath]
    }, options); // Select the P tag at the previous list item

    slate.Transforms.select(editor, slate.Editor.end(editor, previousListItemPath.concat([0])));
  } else {
    ...
    editor.insertBreak = () => {
      ...
      if (res) {
        ...
        if (hasListInListItem(listItemNode)) {
           ...
           */
          if (slate.Editor.isEnd(editor, cursor, listItemPath)) {
            return slate.Transforms.insertNodes(editor, {
              type: li.type,
              children: [{
                type: p.type,
              ...
              at: slate.Path.next(listItemPath),
              select: true
            });
          }
          /**
          ...
          if (slate.Editor.isStart(editor, cursor, listItemPath)) {
            return slate.Transforms.insertNodes(editor, {
              type: li.type,
              children: [{
                type: p.type,
      ...
      if (res) {
        ...
        if (isSelectionAtBlockStart(editor)) {
          slate.Editor.withoutNormalizing(editor, () => {
            ...
            if (moved) return;
            moved = removeRootListItem(editor, {
              list,
              listItem
            }, options);
            if (moved) return;
...
const getClassNames$c = utilities.classNamesFunction();
/**
...
const MentionElementBase = ({
  ...
  }, htmlAttributes), prefix, element.value, children);
};
/**
 * MentionElement
 */

const MentionElement = utilities.styled(MentionElementBase, getMentionElementStyles, undefined, {
...
const getClassNames$d = utilities.classNamesFunction();
const MentionSelectBase = _a => {
  var {
    ...
    onClickMention
      ...
      props = __rest(_a, ["className", "styles", "at", "options", "valueIndex", "onClickMention"]);
  ...
  return /*#__PURE__*/React.createElement(PortalBody, null, /*#__PURE__*/React.createElement("div", Object.assign({
    ref: ref,
    className: classNames.root
  }, props), options.map((option, i) => /*#__PURE__*/React.createElement("div", {
    key: `${i}${option.value}`,
...
const MentionSelect = utilities.styled(MentionSelectBase, getMentionSelectStyles, undefined, {
  scope: 'MentionSelect'
});
...
const insertMention = (editor, mentionable, options) => {
  const {
    mention
  } = setDefaults(options, DEFAULTS_MENTION);
...
const useMention = (mentionables = [], _a = {}) => {
  var {
    ...
    trigger = '@'
      ...
      options = __rest(_a, ["maxSuggestions", "trigger"]);

  const [targetRange, setTargetRange] = React.useState(null);
  const [valueIndex, setValueIndex] = React.useState(0);
  const [search, setSearch] = React.useState('');
  const values = mentionables.filter(c => c.value.toLowerCase().includes(search.toLowerCase())).slice(0, maxSuggestions);
  const onAddMention = React.useCallback((editor, data) => {
    if (targetRange !== null) {
  ...
  const onKeyDownMention = React.useCallback((e, editor) => {
    if (targetRange) {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
  ...
  const onChangeMention = React.useCallback(editor => {
    const {
      selection
    } = editor;

    if (selection && isCollapsed(selection)) {
      const cursor = slate.Range.start(selection);
      const {
        range,
        match: beforeMatch
...
const getClassNames$e = utilities.classNamesFunction();
/**
...
const TableElementBase = ({
  ...
  return /*#__PURE__*/React.createElement("table", Object.assign({}, attributes, {
    className: classNames.root
  }, htmlAttributes), /*#__PURE__*/React.createElement("tbody", null, children));
};
/**
 * TableElement
 */

const TableElement = utilities.styled(TableElementBase, getTableElementStyles, undefined, {
  scope: 'TableElement'
});
...
const getTableCellEntry = (editor, {
  ...
  if (at && isNodeTypeIn(editor, td.type, {
    at
  })) {
    const selectionParent = getParent(editor, at);
    if (!selectionParent) return;
    const [, paragraphPath] = selectionParent;
    const tableCell = getAboveByType(editor, td.type, {
      at
    }) || getParent(editor, paragraphPath);
    if (!tableCell) return;
  ...
  if (isNodeTypeIn(editor, table.type)) {
    ...
    if (currentCellItem && currentTableItem) {
      ...
      currentTableItem[0].children.forEach((row, rowIdx) => {
        ...
        slate.Transforms.insertNodes(editor, getEmptyCellNode(Object.assign({}, options)), {
          at: newCellPath,
...
const addRow = (editor, options) => {
  ...
  if (isNodeTypeIn(editor, table.type)) {
    ...
    if (currentRowItem) {
      ...
      slate.Transforms.insertNodes(editor, getEmptyRowNode(currentRowElem.children.length, options), {
...
const insertTable = (editor, options) => {
  ...
  if (!isNodeTypeIn(editor, table.type)) {
    slate.Transforms.insertNodes(editor, getEmptyTableNode(options));
  }
};
...
const withTable = options => editor => {
  ...
  editor.insertText = text => {
    const {
      selection
    } = editor; // Collapse selection if multiple cells are selected to avoid breaking the table
...
const getClassNames$f = utilities.classNamesFunction();
/**
...
const MediaEmbedElementBase = ({
  ...
  }, /*#__PURE__*/React.createElement("iframe", Object.assign({
    className: classNames.iframe,
    title: "embed",
    src: `${url}?title=0&byline=0&portrait=0`,
    frameBorder: "0"
  }, htmlAttributes))), /*#__PURE__*/React.createElement(MediaEmbedUrlInput, {
...
const MediaEmbedElement = utilities.styled(MediaEmbedElementBase, getMediaEmbedElementStyles, undefined, {
...
const deserializeAlign = options => {
  ...
  return {
    ...
    }, (_b = options === null || options === void 0 ? void 0 : options.align_right) === null || _b === void 0 ? void 0 : _b.deserialize))]
  };
};
...
const renderElementAlign = options => {
  ...
  return getRenderElements([Object.assign({}, align_left), align_center, align_right]);
};
...
const autoformatInlineBlock = (editor, {
  type,
  markup,
  preFormat,
  format
  ...
  e.removeMark = (key, shouldChange = true) => {
    ...
    if (selection) {
      if (slate.Range.isExpanded(selection)) {
        ...
        const marks = Object.assign({}, slate.Editor.marks(editor) || {});
        delete marks[key];
        editor.marks = marks;
...
Object.keys(slatePluginsCore).forEach(function (k) {
  if (k !== 'default') Object.defineProperty(exports, k, {
    enumerable: true,
    get: function () {
      return slatePluginsCore[k];
    }
  });
});
exports.AlignPlugin = AlignPlugin;
exports.BalloonToolbar = BalloonToolbar;
exports.BasicElementPlugins = BasicElementPlugins;
exports.BasicMarkPlugins = BasicMarkPlugins;
exports.BlockquoteElement = BlockquoteElement;
exports.BlockquoteElementBase = BlockquoteElementBase;
exports.BlockquotePlugin = BlockquotePlugin;
exports.BoldPlugin = BoldPlugin;
exports.CLASS_TODO_LIST = CLASS_TODO_LIST;
...
exports.CodeBlockElementBase = CodeBlockElementBase;
exports.CodeBlockPlugin = CodeBlockPlugin;
exports.CodePlugin = CodePlugin;
exports.DEFAULTS_ALIGN = DEFAULTS_ALIGN;
...
exports.DEFAULTS_HEADING = DEFAULTS_HEADING;
exports.DEFAULTS_HIGHLIGHT = DEFAULTS_HIGHLIGHT;
exports.DEFAULTS_IMAGE = DEFAULTS_IMAGE;
exports.DEFAULTS_ITALIC = DEFAULTS_ITALIC;
exports.DEFAULTS_KBD = DEFAULTS_KBD;
exports.DEFAULTS_LINK = DEFAULTS_LINK;
exports.DEFAULTS_LIST = DEFAULTS_LIST;
exports.DEFAULTS_MEDIA_EMBED = DEFAULTS_MEDIA_EMBED;
exports.DEFAULTS_MENTION = DEFAULTS_MENTION;
exports.DEFAULTS_PARAGRAPH = DEFAULTS_PARAGRAPH;
exports.DEFAULTS_SEARCH_HIGHLIGHT = DEFAULTS_SEARCH_HIGHLIGHT;
exports.DEFAULTS_STRIKETHROUGH = DEFAULTS_STRIKETHROUGH;
exports.DEFAULTS_SUBSUPSCRIPT = DEFAULTS_SUBSUPSCRIPT;
exports.DEFAULTS_TABLE = DEFAULTS_TABLE;
exports.DEFAULTS_TODO_LIST = DEFAULTS_TODO_LIST;
exports.DEFAULTS_UNDERLINE = DEFAULTS_UNDERLINE;
exports.DEFAULT_ELEMENT = DEFAULT_ELEMENT;
exports.ELEMENT_ALIGN_CENTER = ELEMENT_ALIGN_CENTER;
exports.ELEMENT_ALIGN_LEFT = ELEMENT_ALIGN_LEFT;
exports.ELEMENT_ALIGN_RIGHT = ELEMENT_ALIGN_RIGHT;
...
exports.MediaEmbedElementBase = MediaEmbedElementBase;
exports.MediaEmbedPlugin = MediaEmbedPlugin;
exports.MentionElement = MentionElement;
exports.MentionElementBase = MentionElementBase;
exports.MentionPlugin = MentionPlugin;
exports.MentionSelect = MentionSelect;
exports.MentionSelectBase = MentionSelectBase;
exports.ParagraphPlugin = ParagraphPlugin;
exports.PortalBody = PortalBody;
exports.PreviewLeaf = PreviewLeaf;
exports.PreviewLeafBase = PreviewLeafBase;
exports.PreviewPlugin = PreviewPlugin;
exports.ResetBlockTypePlugin = ResetBlockTypePlugin;
exports.SearchHighlightPlugin = SearchHighlightPlugin;
exports.Selectable = Selectable;
exports.SoftBreakPlugin = SoftBreakPlugin;
exports.StrikethroughPlugin = StrikethroughPlugin;
exports.StyledComponent = StyledComponent;
exports.StyledComponentBase = StyledComponentBase;
exports.StyledElement = StyledElement;
exports.StyledElementBase = StyledElementBase;
exports.StyledLeaf = StyledLeaf;
exports.StyledLeafBase = StyledLeafBase;
...
exports.TodoListElementBase = TodoListElementBase;
exports.TodoListPlugin = TodoListPlugin;
exports.Toolbar = Toolbar;
exports.ToolbarAlign = ToolbarAlign;
exports.ToolbarBase = ToolbarBase;
exports.ToolbarButton = ToolbarButton;
exports.ToolbarButtonBase = ToolbarButtonBase;
...
exports.autoformatInline = autoformatInline;
exports.autoformatInlineBlock = autoformatInlineBlock;
exports.createDocumentNode = createDocumentNode;
exports.createNode = createNode;
exports.decorateCodeBlock = decorateCodeBlock;
exports.decoratePreview = decoratePreview;
exports.decorateSearchHighlight = decorateSearchHighlight;
exports.defaultsDeepToNodes = defaultsDeepToNodes;
exports.deleteColumn = deleteColumn;
exports.deleteListFragment = deleteListFragment;
exports.deleteRow = deleteRow;
exports.deleteTable = deleteTable;
exports.deserializeAlign = deserializeAlign;
exports.deserializeBlockquote = deserializeBlockquote;
exports.deserializeBold = deserializeBold;
exports.deserializeCode = deserializeCode;
exports.deserializeCodeBlock = deserializeCodeBlock;
exports.deserializeHTMLElement = deserializeHTMLElement;
exports.deserializeHTMLToBreak = deserializeHTMLToBreak;
exports.deserializeHTMLToDocument = deserializeHTMLToDocument;
...
exports.deserializeHeading = deserializeHeading;
exports.deserializeHighlight = deserializeHighlight;
exports.deserializeIframe = deserializeIframe;
exports.deserializeItalic = deserializeItalic;
exports.deserializeKbd = deserializeKbd;
exports.deserializeLink = deserializeLink;
exports.deserializeList = deserializeList;
exports.deserializeMention = deserializeMention;
exports.deserializeParagraph = deserializeParagraph;
exports.deserializeStrikethrough = deserializeStrikethrough;
exports.deserializeSubscript = deserializeSubscript;
exports.deserializeSuperscript = deserializeSuperscript;
exports.deserializeTable = deserializeTable;
exports.deserializeTodoList = deserializeTodoList;
exports.deserializeUnderline = deserializeUnderline;
...
exports.getListItemSublist = getListItemSublist;
...
exports.getMentionSelectStyles = getMentionSelectStyles;
...
exports.getOnHotkeyToggleMark = getOnHotkeyToggleMark;
exports.getOnHotkeyToggleMarkDefault = getOnHotkeyToggleMarkDefault;
exports.getOnHotkeyToggleNodeType = getOnHotkeyToggleNodeType;
exports.getOnHotkeyToggleNodeTypeDefault = getOnHotkeyToggleNodeTypeDefault;
...
exports.getPreviewLeafStyles = getPreviewLeafStyles;
...
exports.getRenderElements = getRenderElements;
exports.getRenderLeaf = getRenderLeaf;
exports.getRenderLeafDefault = getRenderLeafDefault;
exports.getSelectableElement = getSelectableElement;
exports.getSelectableStyles = getSelectableStyles;
exports.getSelectionText = getSelectionText;
exports.getStyledComponentStyles = getStyledComponentStyles;
...
exports.hasListInListItem = hasListInListItem;
...
exports.isNodeTypeIn = isNodeTypeIn;
exports.isNodeTypeList = isNodeTypeList;
...
exports.moveListItemSublistItemsToList = moveListItemSublistItemsToList;
...
exports.onKeyDownResetBlockType = onKeyDownResetBlockType;
...
exports.removeRootListItem = removeRootListItem;
exports.renderElementAlign = renderElementAlign;
exports.renderElementBlockquote = renderElementBlockquote;
exports.renderElementCodeBlock = renderElementCodeBlock;
exports.renderElementHeading = renderElementHeading;
exports.renderElementImage = renderElementImage;
exports.renderElementLink = renderElementLink;
exports.renderElementList = renderElementList;
exports.renderElementMediaEmbed = renderElementMediaEmbed;
exports.renderElementMention = renderElementMention;
exports.renderElementParagraph = renderElementParagraph;
exports.renderElementTable = renderElementTable;
exports.renderElementTodoList = renderElementTodoList;
exports.renderLeafBold = renderLeafBold;
exports.renderLeafCode = renderLeafCode;
exports.renderLeafCodeBlock = renderLeafCodeBlock;
exports.renderLeafHighlight = renderLeafHighlight;
exports.renderLeafItalic = renderLeafItalic;
exports.renderLeafKbd = renderLeafKbd;
exports.renderLeafPreview = renderLeafPreview;
exports.renderLeafStrikethrough = renderLeafStrikethrough;
exports.renderLeafSubscript = renderLeafSubscript;
exports.renderLeafSuperscript = renderLeafSuperscript;
exports.renderLeafUnderline = renderLeafUnderline;
...
exports.unwrapNodesByType = unwrapNodesByType;
exports.upsertAlign = upsertAlign;
...
exports.withTrailingNode = withTrailingNode;
