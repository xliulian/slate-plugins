...
import { Editor, Element, Node as Node$1, Range, Path, Point, Transforms, Text, createEditor } from 'slate';
import React__default, { useState, useRef, forwardRef, useMemo, createElement, useEffect, useCallback } from 'react';
import { ReactEditor, useEditor, useReadOnly, useSlate, Slate, withReact, useFocused, useSelected } from 'slate-react';
import useMergedRef from '@react-hook/merged-ref';
import Tippy from '@tippyjs/react';
import { mergeStyles, concatStyleSets } from '@uifabric/styling';
...
import slate$1 from 'remark-slate';
...
const getAboveByType = (editor, types, options = {}) => {
  ...
  return Editor.above(editor, Object.assign({
...
const getBlockAbove = (editor, options = {}) => Editor.above(editor, Object.assign({
...
const getNodesById = (editor, id, editorNodesOptions) => {
  ...
  const nodes = Editor.nodes(editor, Object.assign({
    mode: 'all',
    match: n => {
      const nodeId = n.id;
...
const getBlocks = (editor, options) => {
  return [...Editor.nodes(editor, Object.assign({
...
const unwrapNodesByType = (editor, types, options = {}) => {
  types = castArray_1(types);
  Transforms.unwrapNodes(editor, Object.assign({
    match: n => types.includes(n.type)
  }, options));
};
...
const ToolbarBase = /*#__PURE__*/forwardRef(({
  className,
  styles,
  children
}, ref) => {
...
const moveListItemUp = (editor, {
  ...
  const newListItemPath = Path.next(listParentPath); // Move item one level up
...
const moveListSiblingsAfterCursor = (editor, {
  ...
  if (!isNodeTypeList(listNode, options) || Path.isParent(at, to) // avoid moving nodes within its own list
  ) {
      return 0;
    }
...
const deleteListFragment = (editor, selection, options = {}) => {
  const [startSelection, endSelection] = Range.edges(selection); // Selection should contain multiple blocks.
  ...
  Editor.withoutNormalizing(editor, () => {
    ...
    if (Path.isBefore(startSelection.path, rootPath)) {
      ...
      next = Path.next(rootPath); // Copy the root list after it.
      ...
      next = listItemSublist ? Path.next(getLastChildPath(listItemSublist)) : listItemStart[1].concat([1, 0]);
    ...
    while (Path.isAfter(cursorPath, startSelection.path)) {
      const node = Node$1.get(editor, cursorPath);
...
const removeRootListItem = (editor, {
  ...
  if (previousListItemPath) {
    const [previousListItemNode] = Editor.node(editor, previousListItemPath); // We may have a trailing sub-list
    ...
    Transforms.select(editor, Editor.end(editor, previousListItemPath.concat([0])));
  } else {
...
export { AlignPlugin, BalloonToolbar, BasicElementPlugins, BasicMarkPlugins, BlockquoteElement, BlockquoteElementBase, BlockquotePlugin, BoldPlugin, CLASS_TODO_LIST, CLASS_TODO_LIST_CHECKED, CODE_BLOCK_LANGUAGES, CodeBlockElement, CodeBlockElementBase, CodeBlockPlugin, CodePlugin, DEFAULTS_ALIGN, DEFAULTS_BLOCKQUOTE, DEFAULTS_BOLD, DEFAULTS_CODE, DEFAULTS_CODE_BLOCK, DEFAULTS_HEADING, DEFAULTS_HIGHLIGHT, DEFAULTS_IMAGE, DEFAULTS_ITALIC, DEFAULTS_KBD, DEFAULTS_LINK, DEFAULTS_LIST, DEFAULTS_MEDIA_EMBED, DEFAULTS_MENTION, DEFAULTS_PARAGRAPH, DEFAULTS_SEARCH_HIGHLIGHT, DEFAULTS_STRIKETHROUGH, DEFAULTS_SUBSUPSCRIPT, DEFAULTS_TABLE, DEFAULTS_TODO_LIST, DEFAULTS_UNDERLINE, DEFAULT_ELEMENT, ELEMENT_ALIGN_CENTER, ELEMENT_ALIGN_LEFT, ELEMENT_ALIGN_RIGHT, ELEMENT_BLOCKQUOTE, ELEMENT_CODE_BLOCK, ELEMENT_H1, ELEMENT_H2, ELEMENT_H3, ELEMENT_H4, ELEMENT_H5, ELEMENT_H6, ELEMENT_IMAGE, ELEMENT_LI, ELEMENT_LINK, ELEMENT_MEDIA_EMBED, ELEMENT_MENTION, ELEMENT_OL, ELEMENT_PARAGRAPH, ELEMENT_TABLE, ELEMENT_TD, ELEMENT_TH, ELEMENT_TODO_LI, ELEMENT_TR, ELEMENT_UL, ExitBreakPlugin, HeadingPlugin, HeadingToolbar, HighlightPlugin, ImageElement, ImageElementBase, ImagePlugin, ItalicPlugin, KbdPlugin, LinkElement, LinkElementBase, LinkPlugin, ListHotkey, ListPlugin, MARK_BOLD, MARK_CODE, MARK_HIGHLIGHT, MARK_ITALIC, MARK_KBD, MARK_PRISM, MARK_SEARCH_HIGHLIGHT, MARK_STRIKETHROUGH, MARK_SUBSCRIPT, MARK_SUPERSCRIPT, MARK_UNDERLINE, MediaEmbedElement, MediaEmbedElementBase, MediaEmbedPlugin, MentionElement, MentionElementBase, MentionPlugin, MentionSelect, MentionSelectBase, ParagraphPlugin, PortalBody, PreviewLeaf, PreviewLeafBase, PreviewPlugin, ResetBlockTypePlugin, SearchHighlightPlugin, Selectable, SoftBreakPlugin, StrikethroughPlugin, StyledComponent, StyledComponentBase, StyledElement, StyledElementBase, StyledLeaf, StyledLeafBase, SubscriptPlugin, SuperscriptPlugin, TableElement, TableElementBase, TableHotKey, TablePlugin, TodoListElement, TodoListElementBase, TodoListPlugin, Toolbar, ToolbarAlign, ToolbarBase, ToolbarButton, ToolbarButtonBase, ToolbarElement, ToolbarImage, ToolbarLink, ToolbarList, ToolbarMark, ToolbarSearchHighlight, ToolbarTable, UnderlinePlugin, addColumn, addRow, applyDeepToNodes, autoformatBlock, autoformatInline, autoformatInlineBlock, createDocumentNode, createNode, decorateCodeBlock, decoratePreview, decorateSearchHighlight, defaultsDeepToNodes, deleteColumn, deleteListFragment, deleteRow, deleteTable, deserializeAlign, deserializeBlockquote, deserializeBold, deserializeCode, deserializeCodeBlock, deserializeHTMLElement, deserializeHTMLToBreak, deserializeHTMLToDocument, deserializeHTMLToDocumentFragment, deserializeHTMLToElement, deserializeHTMLToFragment, deserializeHTMLToMarks, deserializeHTMLToText, deserializeHeading, deserializeHighlight, deserializeIframe, deserializeItalic, deserializeKbd, deserializeLink, deserializeList, deserializeMention, deserializeParagraph, deserializeStrikethrough, deserializeSubscript, deserializeSuperscript, deserializeTable, deserializeTodoList, deserializeUnderline, escapeRegExp, exitBreakAtEdges, filterBreaklines, focusBlockStartById, getAboveByType, getBalloonToolbarStyles, getBlockAbove, getBlockPathById, getBlockquoteElementStyles, getBlocks, getChildren, getCodeBlockElementStyles, getElementDeserializer, getEmptyCellNode, getEmptyRowNode, getEmptyTableNode, getHandler, getHeadingToolbarStyles, getHoverDirection, getImageElementStyles, getLastChild, getLastChildPath, getLastNode, getLeafDeserializer, getLinkElementStyles, getListItemEntry, getListItemSublist, getListNormalizer, getListRoot, getMediaEmbedElementStyles, getMentionElementStyles, getMentionSelectStyles, getNewDirection, getNextIndex, getNextSiblingNodes, getNode, getNodeById, getNodeDeserializer, getNodes, getNodesById, getNodesByType, getNodesRange, getOnHotkeyToggleMark, getOnHotkeyToggleMarkDefault, getOnHotkeyToggleNodeType, getOnHotkeyToggleNodeTypeDefault, getParent, getPointBefore, getPointFromLocation, getPreventDefaultHandler, getPreviewLeafStyles, getPreviousBlockById, getPreviousIndex, getPreviousPath, getRangeBefore, getRangeFromBlockStart, getRenderElement, getRenderElements, getRenderLeaf, getRenderLeafDefault, getSelectableElement, getSelectableStyles, getSelectionText, getStyledComponentStyles, getTableElementStyles, getText, getTodoListElementStyles, getToolbarButtonStyles, getToolbarStyles, grabberTooltipProps, hasListInListItem, htmlStringToDOMNode, insertEmptyElement, insertImage, insertListItem, insertMention, insertTable, isAncestor, isAncestorEmpty, isBlockAboveEmpty, isBlockTextEmptyAfterSelection, isCollapsed, isDescendant, isEnd, isExpanded, isFirstChild, isImageUrl, isListNested, isMarkActive, isNodeType, isNodeTypeIn, isNodeTypeList, isPointAtWordEnd, isSelectionAtBlockEnd, isSelectionAtBlockStart, isSelectionExpanded, isStart, isTextByPath, isUrl, isWordAfterTrigger, mergeDeepToNodes, moveChildren, moveListItemDown, moveListItemSublistItemsToList, moveListItemSublistItemsToListItemSublist, moveListItemUp, moveListSiblingsAfterCursor, normalizeDescendantsToDocumentFragment, normalizeListItem, onImageLoad, onKeyDownExitBreak, onKeyDownList, onKeyDownResetBlockType, onKeyDownSoftBreak, parseMD, pipe, removeBlocksAndFocus, removeFirstListItem, removeRootListItem, renderElementAlign, renderElementBlockquote, renderElementCodeBlock, renderElementHeading, renderElementImage, renderElementLink, renderElementList, renderElementMediaEmbed, renderElementMention, renderElementParagraph, renderElementTable, renderElementTodoList, renderLeafBold, renderLeafCode, renderLeafCodeBlock, renderLeafHighlight, renderLeafItalic, renderLeafKbd, renderLeafPreview, renderLeafStrikethrough, renderLeafSubscript, renderLeafSuperscript, renderLeafUnderline, selectBlockById, selectBlocksBySelectionOrId, selectEndOfBlockAboveSelection, serializeHTMLFromNodes, setDefaults, setPositionAtSelection, toggleList, toggleMark, toggleNodeType, toggleWrapNodes, unhangRange, unwrapList, unwrapNodesByType, upsertAlign, upsertLinkAtSelection, useBalloonMove, useBalloonShow, useDndBlock, useDragBlock, useDropBlockOnEditor, useMention, withAutoformat, withDeserializeHTML, withDeserializeMd, withImageUpload, withInlineVoid, withLink, withList, withMarks, withNodeID, withNormalizeTypes, withRemoveEmptyNodes, withSelectOnBackspace, withTable, withTrailingNode, wrapLink, wrapNodes };
